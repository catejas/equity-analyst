   ...
   && /Income statement/i.test(body) && /Balance sheet/i.test(body) && /Cash flow/i.test(body));
{
  ...
  /* Five years — two reported and three projected — unless the payload cannot
     support a forecast, in which case the report must say so rather than print
     one. THIS payload cannot: it gives PNB's interest expense as 0 and leaves
     employee cost and other expenses null, and a bank's operating lines cannot
     be derived without them. Deriving anyway produced a cost-to-income of 0.8%
     and a credit cost of 85.8% — impossible figures generated by the very code
     written to stop impossible figures.

     So the assertion is: five years, or a stated refusal. Never a forecast
     built on inputs that are not there. */
  const refused = /cannot be derived from this year|no forecast is built/i.test(body);
  ok('five years of statements, or a stated reason there are not',
     (cols.some((x) => !/E$/.test(x)) && cols.some((x) => /E$/.test(x)) && cols.length >= 5)
     || refused);
  ok("and where a bank's lines cannot be derived, it names what is missing",
     !refused || /does not give interest expense/.test(body));
...
/* --- the commentary layer --------------------------------------------- */
{
  ...
  /* "What the forecast assumes" is only owed where there is a forecast. This
     payload cannot support one for a bank, and the report says so instead —
     which is the commentary that situation calls for. */
  const SAYS = ['What the score rests on', 'What the accounting tests found',
    'Why this company cannot enter the Top 3',
    ...(/no forecast is built/i.test(body) ? [] : ['What the forecast assumes']),
  ...
  /* The forecast must name the reported figure it starts from — a base year
     not tied to a filed number is a forecast of something else. PNB is a
     lender, so its forecast is built from its own ratios rather than from the
     driver model, and the anchor it names is interest income rather than
     revenue. Either way the assertion is the same: name the figure. */
  ok('the forecast names the reported figure it is anchored on, or there is none',
     /base year ties to reported interest income of|The base year ties to reported revenue of/
       .test(body) || /no forecast is built/i.test(body));
  ok('and does not report a count of self-checks instead',

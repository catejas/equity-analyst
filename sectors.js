/* sectors.js — a navigation aid, not a registry.
 *
 * This is a taxonomy of where Indian listed companies actually sit, written to
 * make the sector box quick to use on a phone. It is deliberately not a
...
(function (window) {
  ...
  /* Flat searchable index. Each entry is either a sector or a
     sector-and-subSector pair, so one search box serves both. */
  var INDEX = [];
  SECTORS.forEach(function (row) {
    var seg = row[0], subs = row[1] || [];
    INDEX.push({ sector: seg, sub: '', label: seg, kind: 'sector', subCount: subs.length });
    subs.forEach(function (sub) {
      INDEX.push({ sector: seg, sub: sub, label: sub, kind: 'sub' });
    });
  });

  /* Match on word starts as well as substrings, so "elec war" finds
     "Avionics and electronic warfare" and "def" finds the sector before any
     of its subSectors. Ranking: sector before subSector, prefix before
     word-start before substring, then alphabetical. */
  function search(q, limit) {
    q = String(q || '').trim().toLowerCase();
    limit = limit || 60;
    if (!q) {
      return INDEX.filter(function (x) { return x.kind === 'sector'; }).slice(0, limit);
    }
    var terms = q.split(/\s+/).filter(Boolean);
    var out = [];
    INDEX.forEach(function (x) {
      var hay = (x.sector + ' ' + x.sub).toLowerCase();
      ...
      out.push({ item: x, rank: rank + (x.kind === 'sector' ? 0 : 0.5) });
    });
  ...
  function subsOf(sector) {
    var s = String(sector || '').trim().toLowerCase();
    for (var i = 0; i < SECTORS.length; i++) {
      if (SECTORS[i][0].toLowerCase() === s) return SECTORS[i][1].slice();
    }
    return [];
  }
  ...
  window.EQSectors = {
    ...
    search: search,
    subsOf: subsOf,
    count: { sectors: SECTORS.length, entries: INDEX.length }
  };
})(window);

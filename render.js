...
:root{
  /* A4 is 297mm. 263mm leaves 34mm of slack — enough for a browser adding its
     own margins of up to 17mm top and bottom. Safari's default is 12.7mm. */
  --pageh:263mm;
...
/* THE PAGE BOX IS DELIBERATELY SHORTER THAN THE SHEET.
   ...
   It used to be exactly 297mm — the full height of A4 — which is correct only
   when the browser honours a page margin of zero. Safari does not when its
   "print headers and footers" setting is on: it insets the page box to make
   room for the URL and page number, and a 297mm block no longer fits in it.
   A block that does not fit does not clip, it FRAGMENTS — so every page split
   in two and a 24-page report printed as 48, each even sheet carrying nothing
   but the tail of the footer. Measured: two millimetres of page margin is
   enough to double the whole document.

   So the box is sized to survive a browser's default margins (about 12.7mm top
   and bottom) rather than assuming they will be zero. The cost is a band of
   white at the foot of a cleanly printed page; the alternative is a document
   that doubles in length on the device it is actually read on. */
.page{ width:210mm; height:var(--pageh); background:var(--paper); position:relative; overflow:hidden;
       page-break-after:always; break-after:page; display:flex; flex-direction:column; margin:0 auto 8mm; }
...
/* The notice, in full, on every page — which is what it is for.
   ...
   It had been four justified lines at 6.6pt, about 12mm of every sheet. I cut
   it to a one-line summary and moved the full text to the end, and Tejas was
   right to send it back: a research document that circulates page by page
   needs its disclaimer on the page, not twenty-three pages later. The answer
...
function head(p, label){
  ...
  var m = (p && p.meta) || {};
  /* The sector falls through to eqSubject so the header and the masthead
     cannot disagree about what the document is about. */
  var name = S(m.short_name) || S(m.company) || eqSubject(p).sector || '';
...
function buildCompany(p, lang){
  ...
  out += S3('Snapshot') + eqSnapshot(c, lang, p)
    + eqComment(c, 'price') + eqComment(c, 'ownership');
  ...
  out += S3('Scoring') + figScoreBullets(c)
    + tbl(['Model','Score','Coverage'], ['businessQuality','growthMultibagger','valuationOpportunity','riskQuality']
        ...
        { num:[1,2] })
    + (c.ratingsRejected
        ? '<div class="note">' + countOf(c.ratingsRejected, 'rating') + ' arrived without evidence and '
            + plural(c.ratingsRejected, 'was', 'were') + ' not counted.</div>'
        : '');
  ...
  if(projOff2 && lf2 && lf2.available){
    ...
    if(mo.sensitivity && mo.sensitivity.available){
      out += S3('What the answer turns on') + figDrivers(c)
        ...
        + e(sentence(mo.sensitivity.mostSensitiveTo)) + ' '
...
/* "1 ratings arrived without evidence", "1 companies were taken to full
   analysis". Counts were concatenated straight onto plural nouns throughout,
   and on a one-company run — which is most sub-sector runs — the reader met
   the mistake on the first page of three different documents. A count and its
   noun agree here, once, so no call site has to remember. */
function plural(n, one, many){
  var k = Number(n);
  return (k === 1 || k === -1) ? one : (many || (one + 's'));
}
function countOf(n, one, many){
  return (n == null ? 0 : n) + ' ' + plural(n, one, many);
}
/* Join a fragment from the payload onto a sentence without doubling its full
   stop. Research text usually arrives already punctuated, and concatenating
   ". " onto it printed "Adds ~40 bps to ROA structurally.. Expected by: FY27."
   on every thesis in every company report. */
function sentence(txt, fallback){
  var t = S(txt);
  if(!t) return fallback || '';
  return /[.!?…]$/.test(t) ? t : t + '.';
}
/* The unit every money figure in this document is in, for a column header or a
...
/* The document's subject, resolved the same way the running header resolves it.
   These used to disagree: head() read meta.sector while this read run.sector,
   so a run assembled from mismatched pieces printed BANKING — PUBLIC SECTOR
   BANKS along the top of a page titled "Aviation — Airlines". Whatever else is
   wrong upstream, one document now says one thing about what it is. */
/* One resolution, used by the title and by the running header both.

   They used to resolve separately — the header from meta.sector, the title
   from run.sector — and when a run was assembled from mismatched pieces the
   document printed BANKING — PUBLIC SECTOR BANKS along the top of a page
   titled "Aviation — Airlines".

   The run block is preferred because it carries the research's own casing:
   the library record stores the picker's slug, which is lowercase, and reading
   that first put "banking" on the masthead in place of "Banking". meta is the
   fallback for a document built without a run. */
function eqSubject(p){
  var m = eqMeta(p), r = eqRep(p).run || {};
  var sec = S(r.sector) || S(m.sector) || '';
  var sub = S(r.sector) ? S(r.subSector) : S(m.subSector);
  /* The picker's slug is lowercase; a masthead is not. */
  if(sec && sec === sec.toLowerCase()) sec = sec.charAt(0).toUpperCase() + sec.slice(1);
  if(sub && sub === sub.toLowerCase()) sub = sub.charAt(0).toUpperCase() + sub.slice(1);
  return { sector: sec, subSector: sub };
}
function eqTitle(p){
  var s = eqSubject(p);
  return (s.sector || 'Sector') + (s.subSector ? ' — ' + s.subSector : '');
}
/* Replies that answered about another sector, and so were kept out.
   Dropping them quietly would only relocate the confusion — the reader would
   see two companies imported and three in the list, with nothing to explain
   the gap. */
function eqForeignBanner(rep){
  var f = arr(rep && rep.run && rep.run.foreignImports);
  if(!f.length) return '';
  return '<div class="note warn"><b>' + e(f.length === 1 ? 'One imported reply was about a different sector'
      : f.length + ' imported replies were about a different sector') + '</b> '
    + f.map(function(x){
        return e(S(x.name) || S(x.symbol)) + ' — the reply’s run block names '
          + e(S(x.sector)) + (x.subSector ? ' — ' + e(S(x.subSector)) : '')
          + ', not ' + e(S(x.expected)) + '.';
      }).join(' ')
    + ' Research answering a different question cannot be scored against this screen, '
    + 'so it was left out of the study. Re-run the company prompt from this sector’s run, '
    + 'or research it as an independent company.</div>';
}
...
function eqCover(p, lang, docLabel, titleFontPt){
  ...
  return '<div style="height:3mm"></div>'
    ...
    + '<div style="height:2.5mm;background:var(--gold);width:26mm;border-radius:1mm;margin:3mm 0 4mm"></div>'
    /* The three the SCREEN nominated, which is what a sector run produces.
   
       This read rep.top3 — the ranking of companies that have come back from a
       full research run — so a Banking study whose screen had nominated SBI,
       Indian Bank and Bank of Maharashtra opened with "No company cleared the
       kill switch", because the one company researched so far happened to be
       barred. Two different questions were sharing one headline. The screen's
       answer goes on the cover; the research's answer goes where the research
       is reported. */
    + (function(){
        var sc = rep.screen, nom = sc ? arr(sc.top3) : [];
        if(nom.length){
          return '<div class="vb"><div class="h">Nominated by the screen</div><div class="c">'
            + '<div class="v">' + nom.map(function(x){ return e(S(x.name) || S(x.symbol)); }).join(' · ') + '</div>'
            + '<div class="lead" style="margin-top:2mm">'
            + nom.map(function(x){ return e(S(x.symbol) || S(x.name)) + ' ' + n(x.score,1); }).join(' &nbsp;|&nbsp; ')
            + ' &nbsp;·&nbsp; out of ' + countOf((sc.counts && sc.counts.shortlisted) || 0, 'company', 'companies')
            + ' screened</div></div></div>';
        }
        return '<div class="vb"><div class="h">Top 3</div><div class="c">'
          + (t3.length
    ...
    /* The tiles count the screen where there is one, because a sector run's
       own numbers are the shortlist and the ranking, not the one company that
       has since been researched. */
    + (function(){
        var sc = rep.screen, k = (sc && sc.counts) || null;
        if(k){
          return '<div class="tiles" style="margin-top:4mm">'
            + tile('Shortlisted', k.shortlisted || 0, plural(k.shortlisted||0,'company','companies') + ' screened')
            + tile('Fully rated', k.rated || 0, 'rated on all four pillars')
            + tile('Nominated', k.chosen || 0, 'taken to full research')
            + tile('Researched', (c.universe || 0), 'come back in full so far')
            + '</div>';
        }
        return '<div class="tiles" style="margin-top:4mm">'
          + tile('Analysed', (c.universe || 0), plural(c.universe || 0, 'company', 'companies') + ' in the run')
...
/* ===================== THE SCREEN =====================================
   What the sector run actually did, and why these three.

   The sector study used to name three companies and then reproduce the whole
   company report for whichever of them had been researched — so a Banking
   study whose screen nominated SBI, Indian Bank and Bank of Maharashtra
   printed twenty pages about PNB, which was simply the company researched
   next. Tejas: "Sector report talks about PNB company which is not in top 3
   ... ideally sector should talk about the screening funnel and how many
   companies are screened and the criteria/basis of selection process of top 3
   in the report rather than just naming top 3."

   That is right, and it is also the document's whole reason to exist. The
   company reports carry the companies. This carries the screen: how many were
   looked at, what each was rated on, what the rule was, and what the ranking
   came out as — so a reader can disagree with the selection rather than only
   with the selected. */

/* Where the full research disagreed with the screen.
 *
 * The screen rates a company on a page of evidence; the research run rates it
 * on fifty. When the two diverge the divergence is the finding — a company the
 * screen liked and the evidence did not is exactly what a screening process is
 * for, and saying so is the difference between a document that reports a
 * ranking and one that argues about it. */
function eqScreenVersusFull(c, scr){
  if(!scr || scr.score == null) return '';
  var full = c.overall && c.overall.score;
  if(typeof full !== 'number') return '';
  var gap = full - scr.score;
  var keys = ['businessQuality','growthMultibagger','valuationOpportunity','riskQuality'];
  var moved = null, movedBy = 0;
  keys.forEach(function(k){
    var x = scr.ratings && scr.ratings[k];
    var a = (x && typeof x === 'object') ? x.score : x;
    var b = c.pillars && c.pillars[k] && c.pillars[k].score;
    if(typeof a !== 'number' || typeof b !== 'number') return;
    if(Math.abs(b - a) > Math.abs(movedBy)){ movedBy = b - a; moved = k; }
  });
  var label = function(k){
    try{ return (window.EQ && window.EQ.scoring.PILLARS[k] || {}).label || k; }catch(e){ return k; }
  };
  var txt;
  if(Math.abs(gap) < 3){
    txt = 'The full research agrees with the screen to within ' + n(Math.abs(gap),1)
      + ' points, which is inside the noise of either scale. The shortlist rating held up.';
  } else if(gap < 0){
    txt = 'The full research rates this <b>' + n(Math.abs(gap),1) + ' points lower</b> than the '
      + 'screen did — ' + n(full,1) + ' against ' + n(scr.score,1) + '. '
      + (moved ? 'Most of the fall is in ' + e(label(moved)) + ', down '
          + n(Math.abs(movedBy),0) + ' points on the fuller evidence. ' : '')
      + 'A screen rates a company on a page; this rates it on the whole file, and this is what '
      + 'the extra evidence was worth.';
  } else {
    txt = 'The full research rates this <b>' + n(gap,1) + ' points higher</b> than the screen did — '
      + n(full,1) + ' against ' + n(scr.score,1) + '. '
      + (moved ? 'Most of the gain is in ' + e(label(moved)) + ', up '
          + n(Math.abs(movedBy),0) + ' points once the evidence was in. ' : '')
      + 'The shortlist understated it.';
  }
  return '<div class="saybox"><div class="sayh">Screen against full research</div>'
    + '<div class="sayt">' + txt + '</div></div>';
}

function eqScreenBasis(rep){
  ...
  return '<div class="lead"><p>' + e(S(sc.basis)) + '</p></div>'
    + tbl(['What is rated','Weight in the screen'], keys.map(function(k){
        return { cells:[e(pill(k)), Math.round((W[k] || 0) * 100) + '%'] };
      }), { num:[1] })
    + '<p class="note"><b>The application chooses, not the research.</b> The sector run rates '
    + 'the shortlist and this ranks it. Naming three favourites was the one judgement in the '
    + 'system with no rubric behind it, and it is where two AI tools given identical facts '
    + 'returned different threes. The same ratings now always produce the same three, and the '
    + 'reason each one is there can be read off the table below.</p>';
}

/* The ranking itself, every company on it, with the evidence behind each
   rating available to the reader rather than summarised away. */
function eqScreenTable(rep){
  var sc = rep && rep.screen;
  if(!sc || !arr(sc.ranked).length) return '';
  var keys = ['businessQuality','growthMultibagger','valuationOpportunity','riskQuality'];
  var val = function(r, k){
    var x = r.ratings && r.ratings[k];
    var v = (x && typeof x === 'object') ? x.score : x;
    return (typeof v === 'number') ? n(v, 0) : '<span class="mut">—</span>';
  };
  var chosen = {};
  arr(sc.top3).forEach(function(x){ chosen[S(x.symbol) || S(x.name)] = true; });

  var rows = arr(sc.ranked).map(function(r){
    var mine = chosen[S(r.symbol) || S(r.name)];
    var nm = e(S(r.name) || S(r.symbol));
    return { __cls: mine ? 'tot' : '', cells:[
      '<span class="en">' + r.rank + '</span>',
      mine ? '<b>' + nm + '</b>' : nm
    ].concat(keys.map(function(k){ return val(r, k); }))
     .concat([ r.score == null ? '<span class="mut">not ranked</span>'
        : (mine ? '<b>' + n(r.score,1) + '</b>' : n(r.score,1)),
       r.sufficient ? (mine ? '<b>nominated</b>' : 'rated on all four')
         : '<span class="mut">rated on ' + r.rated + ' of 4</span>' ]) };
  });
  return tbl(['#','Company','Business','Growth','Valuation','Risk','Screen','Standing'],
    rows, { num:[2,3,4,5,6], chunk:14 });
}

/* Why each of the three is there, in its own words and its own numbers. */
function eqScreenChosen(rep){
  var sc = rep && rep.screen;
  if(!sc || !arr(sc.top3).length) return '';
  var keys = ['businessQuality','growthMultibagger','valuationOpportunity','riskQuality'];
  var pill = function(k){
    try{ return (window.EQ && window.EQ.scoring.PILLARS[k] || {}).label || k; }catch(e){ return k; }
  };
  /* Where a company has also been researched in full, its reported financials
     are shown beside the screen that picked it — which is the comparison a
     reader wants and the one the document never offered. */
  var researched = {};
  arr(rep.full).forEach(function(c){ researched[String(c.symbol||'').toUpperCase()] = c; });

  return arr(sc.top3).map(function(r, i){
    var ratings = r.ratings || {};
    var ev = keys.map(function(k){
      var x = ratings[k];
      var v = (x && typeof x === 'object') ? x.score : x;
      var why = (x && typeof x === 'object') ? S(x.evidence) : '';
      if(typeof v !== 'number') return null;
      return { cells:[ e(pill(k)), '<b>' + n(v,0) + '</b>',
        why ? e(why) : '<span class="mut">no evidence was given for this rating</span>' ] };
    }).filter(Boolean);

    var full = researched[String(r.symbol||'').toUpperCase()];
    var fin = '';
    if(full){
      var ann = arr(full.financials && full.financials.annual);
      var last = ann.length ? ann[ann.length-1] : null;
      if(last){
        var lender = !!full.lender;
        fin = '<div style="height:2mm"></div>'
          + tbl(['As reported, ' + e(S(last.period || 'latest year')), 'Value'], [
              { cells:[lender ? 'Interest income' : 'Revenue', eqNum(last.revenue,0)] },
              { cells:[lender ? 'Pre-provision operating profit' : 'EBITDA',
                  eqNum(lender ? last.ebit : last.ebitda, 0)] },
              { cells:['Profit after tax', eqNum(last.netProfit,0)] },
              { cells:['EPS diluted', eqNum(last.epsDiluted,2)] }
            ], { num:[1] });
      }
    }

    return '<div class="ir-box" style="margin-bottom:3mm">'
      + '<h4>' + (i+1) + '. ' + e(S(r.name) || S(r.symbol))
        + ' <span class="mut">— screen ' + n(r.score,1) + ' of 100</span></h4>'
      + (S(r.why) ? '<p class="lead">' + e(S(r.why)) + '</p>' : '')
      + tbl(['Rated on','Score','The evidence behind it'], ev, { num:[1] })
      + fin
      + (full ? '' : '<p class="note">Not yet researched in full. The screen nominates it; the '
          + 'company research run is what tests it.</p>')
      + '</div>';
  }).join('');
}

/* What the screen could not settle. A ranking with ties inside the noise band
   is a ranking the reader should not read as an ordering. */
function eqScreenTies(rep){
  var sc = rep && rep.screen;
  if(!sc || !arr(sc.ties).length) return '';
  return '<div class="note warn"><b>' + countOf(sc.ties.length, 'pair') + ' too close to separate.</b> '
    + arr(sc.ties).map(function(t){
        return e(S(t.a)) + ' and ' + e(S(t.b)) + ' are ' + n(t.gap,1) + ' points apart';
      }).join('; ')
    + '. A gap that small is inside the noise of any rating scale. Treat them as equally '
    + 'placed rather than as ranked against each other.</div>';
}

function eqFunnel(p, lang){
  var rep = eqRep(p), u = rep.universe || {}, c = rep.counts || {};
  /* The funnel is the screen's funnel. "Shortlisted for analysis" read
     counts.universe — the companies that have come back from a full research
     run — so a twelve-bank screen drew a funnel reading 1, 1, 1, 0 and told
     the reader the sector contained one company. Every stage the screen owns
     now comes from the screen, and the research stages are named as such. */
  var sc = rep.screen, k = (sc && sc.counts) || null;
  var rows = [
    { cells:['Identified in the sector', u.identified == null ? '&mdash;' : n(u.identified,0)] },
    { cells:['Shortlisted and rated', n(k ? k.shortlisted : (c.universe || 0), 0)] }
  ];
  if(k){
    rows.push({ cells:['Rated on all four pillars', n(k.rated || 0, 0)] });
    rows.push({ cells:['Nominated for full research', n(k.chosen || 0, 0)] });
  }
  rows.push({ cells:['Researched in full so far', n(c.universe || 0, 0)] });
  rows.push({ cells:['Cleared the coverage floor', n(c.scored || 0, 0)] });
  rows.push({ cells:['Eligible for the Top 3', n(c.top3Eligible || 0, 0)] });
  rows.push({ cells:['Barred by the kill switch', n((rep.excludedFromTop3 || []).length, 0)] });
...
function figFunnel(p){
  ...
  var rep=eqRep(p), u=rep.universe||{}, c=rep.counts||{};
  /* The screen's own stages, where there is a screen. Drawing the funnel from
     the research counts gave a twelve-bank sector a funnel of 1, 1, 1, 0. */
  var k = (rep.screen && rep.screen.counts) || null;
  var steps = k
    ? [ { label:'Identified in the sector', value:u.identified },
        { label:'Shortlisted and rated', value:k.shortlisted },
        { label:'Rated on all four', value:k.rated },
        { label:'Nominated', value:k.chosen },
        { label:'Researched in full', value:c.universe } ]
    : [ { label:'Identified in the sector', value:u.identified },
        { label:'Taken to analysis', value:c.universe },
        { label:'Scored', value:c.scored },
        { label:'Cleared the kill switch', value:c.top3Eligible } ];
  return K.funnel({ title:'Screening funnel, companies', source:'This run',
    steps: steps.filter(function(s){ return typeof s.value==='number'; }) });
}
...
function eqTearSheet(p, c, lang){
  ...
  /* Every row here is in a different unit — a market cap in crore, a price in
  ...
  addStat('Against ' + e(S(perf.benchmark || 'the index')),
    isNum_(perf.m12Relative) ? eqSignedPct(perf.m12Relative) : null);
  ...
  var fin = (function(){
    ...
    /* Plain text: tbl() escapes its header cells, so markup passed here is
       printed literally — which is how <SPAN CLASS="EN">FY24</SPAN> ended up
       on the front page of the report. */
    var head2 = [''].concat(rows.map(function(r){ return S(r.period || ''); }));
...
function figSensitivity(c){
  ...
  if(cells.length > 1){
    ...
    note = 'Each cell is the value per share at that discount rate and that terminal growth rate — '
      ...
      + n(lo,0) + ' to ' + n(hi,0) + '.'
      + (above == null ? ''
         : ' ' + above + ' of the ' + countOf(cells.length, 'combination') + ' '
             + plural(above, 'sits', 'sit') + ' above the current price of '
           + n(price,2) + (above === 0
...
function figMultibagger(c){
  ...
  return K.columns({ title:'Compound return required for each multiple',
    ...
    /* An axis of bare numbers where every number is a rate of return. */
    yFmt:function(v){ return n(v,0) + '%'; },
...
function figGlobalGrowth(rep){
  ...
  /* The axis carries its unit. It used to print bare numbers, so a reader met
     "0.16" with nothing to say whether that was sixteen percent or a sixth of
     one. The engine now rescales a block that arrived as fractions and records
     it as a gap; this makes sure the axis states what it is either way. */
  return K.columns({ title:'Global market growth, by lookback', source:S(g.source),
    yFmt:function(t){ return n(t,1) + '%'; },
...
function figShareholding(c){
  ...
  return K.columns({ title:'Shareholding, quarter by quarter', source:'Exchange filings',
    stacked:true, yFmt:function(v){ return n(v,0) + '%'; },
    categories:qs.map(function(x){ return S(x.period); }),
...
function eqRotation(p, lang){
  ...
  var rows = (r.ranked || []).map(function(x, i){
    return { cells:[ String(i + 1), e(S(x.sector + (x.subSector ? ' \u2014 ' + x.subSector : ''))),
      ...
      '<span class="mut">' + x.withMomentum + ' of ' + countOf(x.companies, 'company', 'companies')
        + ' priced</span>' ] };
  });
  return '<p>' + (r.reliable
...
function eqTheses(c, lang){
  ...
  return t.map(function(x,i){
    return '<div class="ir-box"><h4>Thesis ' + (i+1) + '. ' + e(S(x.claim)) + '</h4>'
      ...
      + ((S(x.size)||S(x.by))
          ? '<div class="mut">Size: ' + e(sentence(x.size, 'Not quantified.'))
            + ' Expected by: ' + e(sentence(x.by, 'not dated')) + '</div>' : '')
      + '</div>';
  }).join('');
}
...
/* The screen's reading takes the whole report, not a company. */
function eqScreenComment(rep){
  try{
    if(!window.EQ || !EQ.commentary || typeof EQ.commentary.screenCommentary !== 'function') return '';
    return eqSay(EQ.commentary.screenCommentary(rep));
  }catch(err){ return ''; }
}
...
function eqComment(c, key){
  try{
    ...
    var fn = { score:'scoreCommentary', forensic:'forensicCommentary',
      ...
      valuation:'valuationCommentary', multibagger:'multibaggerCommentary',
...
function eqThreeStatement(c, p){
  ...
  var note = nHist + ' reported ' + (nHist === 1 ? 'year' : 'years')
    ...
    + '. Reported figures are as filed; forecast figures come from '
  ...
  /* The unit goes in the corner of the table, where a reader looks for it on
...
function buildExec(p, lang){
  ...
  out += sec('01', 'The run')
    + '<div class="lead"><p>' + e(eqTitle(p)) + ' over a horizon of ' + e(S(run.horizon))
      + '. ' + countOf(rep.counts ? rep.counts.universe : 0, 'company', 'companies') + ' analysed, '
  ...
  out += sec('04', 'Ranking') + eqForeignBanner(rep) + figTop3(rep) + eqRankTable(p, rep.top10, lang);
  ...
  var head = '<div class="sc-top"><div style="height:4mm"></div>'
    ...
    + '<div class="tiles">'
      ...
      + tile('Coverage', (cover == null ? '&mdash;' : Math.round(cover*100) + '<small>%</small>'),
             (co.ratingsRejected ? countOf(co.ratingsRejected, 'rating') + ' not counted' : 'all ratings evidenced'))
  ...
  function forensicBlock(){
    ...
    if(arr(f.notComputed).length){
      rows.push({ cells:[
        ...
        /* Each entry is { name, reason }, not a string. Joining them printed
           "[object Object]; [object Object]" on the score card — and worse,
           printed it where a reader was owed the reason a forensic test could
           not be run. Which test, and why, is the whole content of this row. */
        '<span class="mut">' + arr(f.notComputed).map(function(nc){
          if(nc == null) return '';
          if(typeof nc === 'string') return e(nc);
          var nm = S(nc.name), why = S(nc.reason);
          return nm && why ? e(nm) + ' — ' + e(why) : e(nm || why);
...
function packDoc(p, lang, cfg){
  ...
  var CSS2 = (cfg.extraCss || '')
    ...
    /* The unit rides on the section label, lighter and not uppercased, so the
  ...
  var FIT = '<script>(function(){'
    ...
    + 'function cut(only, i, A){'
      ...
      /* A figure is indivisible.

         The descent used to walk straight into one, because a block holding a
         single chart is a one-child chain all the way down. It then divided
         the figure by its own children and carried the chart to the next page
         while its caption or its source line stayed behind — the peer scatter
         printed on page 16 with "Source: Company filings and exchange data"
         alone at the top of page 17. A chart, its caption, its note and its
         source are one object; they move together or not at all. */
      + 'while(host.children.length===1 && depth++<6){'
        + 'if(host.children[0].tagName==="FIGURE") break;'
        + 'host=host.children[0];'
      + '}'
      + 'if(host.tagName==="FIGURE") return false;'
      ...
      /* Tables are moved whole, never divided by the row.
     
         Dividing them was how the header got lost: the half carried away was a
         shallow clone of the table, and every attempt to give it back its
         column headings — at the cut, by cut id, by proximity — either missed
         or labelled the rows with the wrong table's header. tbl() already
         emits a long table as a run of chunks, each one repeating the header,
         so the packer can divide a block between chunks and never has to take
         a table apart. A table that is genuinely taller than a page falls
         through to the scale guard, which is what it is for. */
      ...
      + 'if(host.tagName==="TABLE"){ return false; }'
      ...
      + 'else if(host.children.length<=keep){'
        ...
        + 'while(h2 && h2.tagName!=="FIGURE" && h2.children.length && h2.children.length<2 && d2++<4)'
          + 'h2=h2.children[h2.children.length-1];'
        /* The same rule as above, enforced on the last-resort descent too.
           A figure has a caption, a body, a note and a source — four children,
           which looks divisible and is not. This is the path that actually
           split the peer scatter. */
        + 'if(!h2 || h2.tagName==="FIGURE" || h2.querySelector("figure")) return false;'
        /* Not a table either. This descent lands on the TABLE itself when a
           section is a heading and one table, and the move loop below then
           carries the tbody away and leaves the thead standing alone — the
           column header at the foot of one page and its rows at the top of the
           next, which is how the sector study printed. Tables move whole. */
        + 'if(h2.tagName==="TABLE" || h2.tagName==="TBODY") return false;'
        + 'if(h2.children.length<2) return false;'
        + 'host=h2; keep=1;'
      + '}'
      ...
      /* Same record as the other cut path: name the two halves of a table
         divided by the row, so the header can be repeated on the lower one
         without anything having to guess which table it belongs to. */
      + 'var g3=0;'
      ...
      /* A heading goes with what it introduces. The loop above moves whole
         children off the end, and when the thing it moved was a table its
         heading stayed behind — "Global peers" sat alone at the foot of a page
         with its table overleaf. Any heading left trailing follows the block
         it names. */
      + 'function isHead(el){ return el && (/^H[1-6]$/.test(el.tagName)'
        + ' || / (sec|ir-grp|sc-h) /.test(" "+el.className+" ")); }'
      + 'var g3b=0;'
      + 'while(host.children.length>1 && isHead(host.children[host.children.length-1])'
        + ' && g3b++<8){'
        + 'cur.insertBefore(host.children[host.children.length-1], cur.firstChild); }'
      ...
      + 'if(thin || boxes[i].scrollHeight>A){'
        + 'var cands=only.querySelectorAll(".grid4,.grid3,.grid2,ul,ol"), qi=0;'
    ...
    + 'for(var s1=0;s1<boxes.length-1;s1++){'
      ...
      + '}'
      + 'if(!ph) continue;'
      /* Never hand rows between tables. This loop exists to repair a table the
         packer had divided — and now it never divides one, so any pair it
         finds here is two DIFFERENT tables that happen to sit either side of a
         break. Moving rows between them is how a headless table appeared after
         every other route to one had been closed. */
      + 'if(nh.tagName==="TBODY" || ph.tagName==="TBODY" || nh.tagName==="TABLE") continue;'
      + 'var g6=0;'
    ...
    /* A header with nothing under it.
       ...
       The row handback can leave a table whose every body row went to the next
       page, and the half above is then a column header standing over a page
       break — MEASURE / VALUE at the foot of one page and its six readings at
       the top of the next, which is how the sector study printed. It reads as a
       rendering fault, and the reader loses the header the rows belong to.

       So an empty-bodied table is not left on the page. Where the rows continue
       directly below, the header is handed down to travel with them; where they
       do not, the whole block moves. Either way no page ends on a header. */
    + 'for(var s2=0;s2<boxes.length-1;s2++){'
      + 'var tbl2=[].slice.call(boxes[s2].querySelectorAll("table"));'
      + 'for(var ti=0;ti<tbl2.length;ti++){'
        + 'var T=tbl2[ti], bd2=T.tBodies;'
        + 'if(bd2 && bd2.length && bd2[0].rows.length) continue;'
        /* Where its rows went is answered by document order, not by the cut
           ids: the continuation is simply the next table below, and it is the
           one missing a header. Matching on ids was fragile — the id sits on
           whichever ancestor the cut chose — and when the lookup missed, the
           header was dropped instead of handed down, which left six unlabelled
           readings at the top of the page. */
        + 'var below=null, all=document.querySelectorAll("table"), seen=false;'
        + 'for(var ai=0;ai<all.length;ai++){'
          + 'if(all[ai]===T){ seen=true; continue; }'
          + 'if(!seen) continue;'
          + 'var bd3=all[ai].tBodies;'
          + 'if(bd3 && bd3.length && bd3[0].rows.length){ below=all[ai]; break; }'
        + '}'
        + 'if(below && T.tHead && !below.tHead'
          + ' && below.rows.length && T.tHead.rows[0].cells.length===below.tBodies[0].rows[0].cells.length){'
          + 'below.insertBefore(T.tHead, below.firstChild);'
        + '}'
        + 'var blk=T; while(blk.parentNode && blk.parentNode!==boxes[s2]) blk=blk.parentNode;'
        + 'if(T.parentNode) T.parentNode.removeChild(T);'
        /* A wrapper left holding nothing but the table that just went is
           empty space with a border round it. */
        + 'if(blk.parentNode===boxes[s2] && !blk.querySelector("table,svg,img")'
          + ' && !blk.textContent.trim()) blk.parentNode.removeChild(blk);'
      + '}'
    + '}'
...
function buildSector(p, lang){
  ...
  /* ---------------- the mandate ---------------- */
  var sc0 = rep.screen;
  var nShort = (sc0 && sc0.counts && sc0.counts.shortlisted) || 0;
  out += S2('The mandate')
    + eqForeignBanner(rep)
    + '<div class="lead"><p>' + e(eqTitle(p)) + ', over a holding horizon of '
      + e(S(run.horizon)) + '. '
      + (nShort
          ? 'The Indian listed universe was screened for this sector and '
            + countOf(nShort, 'company', 'companies') + ' shortlisted. Each was rated on '
            + 'four pillars against written anchors and ranked; the '
            + countOf((sc0.counts.chosen || 0), 'company', 'companies')
            + ' at the top of that ranking '
            + plural(sc0.counts.chosen || 0, 'is', 'are') + ' nominated for full research. '
            + 'What follows is that screen — the universe, the basis, the ranking and the '
            + 'three it produced — followed by the sector this all sits in.'
          : 'The Indian listed universe was screened for this sector.')
      + (cts.universe
          ? ' ' + countOf(cts.universe, 'company', 'companies') + ' '
            + plural(cts.universe, 'has', 'have') + ' since come back from a full research run'
            + (cts.top3Eligible != null
                ? ' and ' + (cts.top3Eligible || 0) + ' cleared the kill switch' : '') + '.'
          : '')
      + '</p></div>'

    /* THE SCREEN, before anything else.
   
       This is what a sector study is. It used to be absent entirely: the
       document named three companies and then reproduced a full company report
       for whichever of them had been researched, so the reader could see the
       conclusion and none of the work behind it. */
    + (sc0 ? S2('How the three were chosen') + eqScreenBasis(rep)
        + S2('The screen, ranked') + eqScreenTable(rep) + eqScreenComment(rep) + eqScreenTies(rep)
        + S2('The three nominated, and why') + eqScreenChosen(rep) : '')
  ...
  /* ---------------- the researched companies, in brief ----------------

     This used to reproduce the entire company report — snapshot, theses,
     forecast, sensitivity, valuation, moat, management, peers, ESG, forensic,
     registers, liquidity, risks, catalysts, monitoring and sources — for every
     company that had come back from a full run. Twenty pages of it, inside a
     document about a sector, for a company that was not even one of the three.

     The company report already exists and is a separate document. What belongs
     here is the bridge between the two: what the full research found, set
     against what the screen expected, in one block per company. Where they
     disagree is the interesting part, and it was invisible while the sector
     study was busy being a second company report. */
  ...
  top.forEach(function(c){
    ...
    out += S2(nm + ' — what the full research found');

    /* The four pillars, scored on the full evidence, beside the same four
       scored on the screen's. A company the screen liked and the research did
       not is the single most useful line a sector study can carry. */
    var scr = null;
    if(rep.screen) arr(rep.screen.ranked).forEach(function(r){
      if(String(r.symbol || '').toUpperCase() === String(c.symbol || '').toUpperCase()) scr = r;
    });
    var keys = ['businessQuality','growthMultibagger','valuationOpportunity','riskQuality'];
    out += tbl(['Model','On the screen','On full research','Coverage'],
        keys.map(function(k){
          var pl = (window.EQ && window.EQ.scoring.PILLARS[k]) || { label:k };
          var s2 = c.pillars && c.pillars[k];
          var sv2 = null;
          if(scr){ var x = scr.ratings && scr.ratings[k];
            sv2 = (x && typeof x === 'object') ? x.score : x; }
          return { cells:[ e(pl.label),
            (typeof sv2 === 'number') ? n(sv2,0) : '<span class="mut">not screened</span>',
            '<b>' + eqNum(s2 && s2.score) + '</b>',
            s2 && s2.coverage != null ? Math.round(s2.coverage*100) + '%' : '&mdash;'] };
        }).concat([{ __cls:'tot', cells:['<b>Overall</b>',
          scr && scr.score != null ? n(scr.score,1) : '<span class="mut">—</span>',
          '<b>' + eqNum(c.overall && c.overall.score) + '</b>',
          c.overall && c.overall.coverage != null
            ? Math.round(c.overall.coverage*100) + '%' : '&mdash;'] }]),
        { num:[1,2,3] })
    ...
    if(arr(c.theses).length){
      out += '<div class="lead">' + arr(c.theses).map(function(t, j){
        return '<p style="margin-bottom:1.4mm"><b>' + (j+1) + '.</b> ' + e(S(t.claim))
          + (S(t.evidence) ? ' <span class="mut">' + e(S(t.evidence)) + '</span>' : '') + '</p>';
      }).join('') + '</div>';
    }
    if(arr(c.thesisBreakers).length){
      out += '<div class="mut" style="margin-top:1.5mm"><b>What would break it.</b></div>'
        + eqList(c.thesisBreakers.slice(0, 5));
    }
    if(arr(c.exclusionReasons).length){
      out += '<div class="note warn"><b>Barred from the Top 3.</b> '
        + arr(c.exclusionReasons).map(function(r){ return e(S(r)); }).join(' ') + '</div>';
    }
    out += '<p class="note">The full argument, the driver model, the valuation and the '
      + 'accounting review are in the ' + e(nm) + ' research report.</p>';
  });
...
global.EQDocs = { buildCompany:buildCompany, buildExec:buildExec,
                   ...
                   charts:{ financials:chartFinancials, gauge:chartGauge,
                   ...
                   buildScorecard:buildScorecard, S:S,

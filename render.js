...
var T = {
  ...
  /* One line, for the foot of every page. The full notice above is printed
     once, at the end, rather than twenty-four times. */
  footnoteShort: 'Research tool only — not investment advice. Not SEBI-registered. Verify every figure against the filings.',
};;
...
/* The footer carried the whole legal notice on EVERY page — four justified
   lines at 6.6pt, about 12mm of every sheet given to the same paragraph 24
   times. It is now one short line per page, with the full notice printed once
   at the end of the document where a reader can actually find it. */
.rfn{ padding:1.2mm 15mm 0; font-size:5.6pt; line-height:1.3; color:var(--ink4);
      white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
body.gu .rfn{ font-size:5.4pt; }
...
/* The notice in full, once, at the foot of the last page. */
.rfull{ padding:2mm 15mm 0; font-size:5.8pt; line-height:1.4; color:var(--ink4);
        text-align:justify; }
 ...
 * the footer pushing a 297mm box past the sheet and doubling the page count.
 * But the reminder is not the notice: it drops who prepared this, that they
 * are not SEBI-registered, and that it is not an offer. So the full text is
 * printed once, at the end, where a reader who has finished will meet it.
 *
 * This runs from the shared tail rather than from one document's packer: the
 * score card has a packer of its own, and placing it in the other one meant
 * that document carried only the reminder. */
+ 'var __live = [].slice.call(document.querySelectorAll(".page"));\n'
+ 'var __full = document.querySelector("[data-fullnotice]");\n'
+ 'if(__live.length && __full && !document.querySelector(".rfull")){\n'
+ '  var __txt = __full.getAttribute("data-fullnotice");\n'
   /* The last page, or the nearest one before it with room above its footer. */
+ '  for(var __lp = __live.length - 1; __lp >= Math.max(0, __live.length - 3); __lp--){\n'
+ '    var __pg = __live[__lp], __foot = __pg.querySelector(".rfw");\n'
+ '    if(!__foot) continue;\n'
+ '    var __n = document.createElement("div");\n'
+ '    __n.className = "rfull"; __n.textContent = __txt;\n'
+ '    __foot.parentNode.insertBefore(__n, __foot);\n'
+ '    var __bd = __pg.querySelector(".body");\n'
   /* .page is overflow:hidden, so a notice that does not fit is a notice that
      silently is not there. Take it back out and try the page before. */
+ '    if(__bd && __bd.scrollHeight > __bd.clientHeight + 1){ __n.parentNode.removeChild(__n); }\n'
+ '    else break;\n'
+ '  }\n'
   /* Some documents are packed to the millimetre on every page — the score
      card fills all four of its bodies exactly — so there is no room above any
      footer. Rather than let those carry the reminder alone, the last page\'s
      footer line itself becomes the notice in full. It wraps to two or three
      lines on ONE page, which the 34mm of page slack absorbs. */
+ '  if(!document.querySelector(".rfull")){\n'
+ '    var __ln = __live[__live.length - 1].querySelector(".rfn");\n'
+ '    if(__ln){\n'
+ '      __ln.textContent = __txt;\n'
+ '      __ln.style.whiteSpace = "normal"; __ln.style.overflow = "visible";\n'
+ '      __ln.style.textOverflow = "clip"; __ln.style.fontSize = "5.2pt";\n'
+ '      __ln.style.lineHeight = "1.35";\n'
+ '    }\n'
+ '  }\n'
+ '}\n'
...
function foot(p, i, total, lang, docName){
  return '<div class="rfw"'
       /* The full notice travels with the footer, unprinted, so the packer can
          place it once on whichever page turns out to be the last. */
       + ' data-fullnotice="'+e(L(lang,'footnote'))+'"'
       + '>'
       + '<div class="rfn">'+e(L(lang,'footnoteShort') || L(lang,'footnote'))+'</div>'
...
function buildSector(p, lang){
  ...
  out += S2('The mandate')
    ...
    + '<div class="lead"><p>' + e(eqTitle(p)) + ', over a holding horizon of '
      ...
      + countOf(cts.universe || 0, 'company', 'companies') + ' '
      + plural(cts.universe || 0, 'was', 'were') + ' taken to full analysis and '
    ...
    if(mo && mo.model && mo.model.available){
      ...
      if(mo.sensitivity && mo.sensitivity.available){
        out += tbl(['Driver','Change','Cumulative FCFF','Terminal EPS'],
          ...
          + '<div class="mut" style="margin-top:1.5mm">Most sensitive to '
          + e(sentence(mo.sensitivity.mostSensitiveTo)) + ' ' + e(S(mo.sensitivity.note)) + '</div>';
      }
      if(mo.valuation && mo.valuation.available){
        out += tbl(['Measure','Value'], [
            { cells:['Value per share', '<b>' + n(mo.valuation.perShare,2) + '</b>'] },
